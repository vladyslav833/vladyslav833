<script type="text/javascript" src="{$homeUrl}js/prj_calendar_scripts.js"></script>
...
<div class="row">
    <form method="post" action="{$adminUrl}prj-calendar" onsubmit="return validate()">
        <div class="col-sm-12">
                ...
                <div class="filter-section">
                    ...
                    <div id="user_role_filter">
                        <select class="form-control es-select w-200" name="prjselect" id="prjselect" required>
                            <option value="0" {if !$filters.prjselect} selected{/if}>Select Job</option>
                            {foreach from=$jobs item=job}
                                <option value="{$job.id}" {if $job.id == $filters.prjselect} selected{/if}>{$job.name}</option>
                            {/foreach}
                        </select>
                    </div>
                </div>
    ...
    <div class="col-sm-12">
        <div class="subpadding20">
            <h2 class="prj-name" id="prj-name" name="prj-name" style="color: #003f6e;">{$selected_job}</h2>
        </div>
    </div>
    <div class="col-sm-12">
        <div class="subpadding20">
            <div class="clr20"></div>
            <div class="calendar-table-wrap" style="display: {($selected_job)?"block":"none"}">
                <div class="table-content">
                    {if count($workers) > 0 || count($reservations) > 0}
                    <table class="display table cell-border table-bordered" id="calendar-table">
                        <tbody>
                            ...
                            <tr class="text-center text-bold">
                                <td class="gray-back category-cell headcol fs-18">{if count($workers) > 0}Workers{/if}</td>
                                {foreach from=$filters.wname_headers item=value}
                                    <td class="text-center text-uppercase">{$value}</td>
                                {/foreach}
                            </tr>
                            {if count($workers) > 0}
                                {foreach from=$workers item=scheduleValue key=key name=workerarr}
                                <tr class="text-center">
                                    <td class="category-cell text-bold headcol fs-18">{$key}</td>
                                    {foreach from=$scheduleValue item=sValue}
                                        {if $sValue == 1}
                                            <td class="filled-cell-{fmod($smarty.foreach.workerarr.index, 4)}"></td>
                                        {else}
                                            <td></td>
                                        {/if}
                                    {/foreach}
